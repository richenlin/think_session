...
module.extends = class {
    ...
    set(ctx, name, value, timeout) {
        try {
            ...
            let data = instance.get(instance.cache_key_prefix + instance.session_key);
            if (!lib.isEmpty(data)) {
            ...
            return instance.set(instance.cache_key_prefix + instance.session_key, data, timeout);
        } catch (e) {
    ...
    get(ctx, name) {
        try {
            ...
            if (data.expire && Date.now() > data.expire) {
                instance.rm(instance.cache_key_prefix + instance.session_key);
                return '';
            } else {
                return data[name];
            }
    ...
    rm(ctx, name) {
        try {
            ...
            let instance = this.getInstance(ctx);
            return instance.rm(instance.cache_key_prefix + instance.session_key);
        } catch (e) {
            return Promise.resolve();
        }
    }
